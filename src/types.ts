...
/* ---------- КЛАССИЧЕСКИЕ ПРЕСЕТЫ РЕЖИМОВ (редактор карт, верхняя панель) ----------
   ...
   помечается «ИЗМЕНЕННЫЙ» (mapModeModified). Стандарты (v0.43.0):
...
export const challengeSummaryLines = (a: ChallengeAnswers): string[] => {
  ...
  const res = [a.resTime ? 'время' : '', a.resTries ? 'попытки' : '', a.resCoins ? `монеты (старт ${coinsStr(a.startCoins)})` : '', a.resHp ? 'полоска HP (+10% за победу / −5% за поражение)' : ''].filter(Boolean).join(' + ') || 'без ресурсов';
...
export interface GameMap {
  ...
  resMode?: 'std' | 'coins' | 'hp'; // РЕСУРС ПАРТИИ — ВЫБОР ОДНОГО ИЗ ТРЁХ (взаимоисключающий): «время и попытки» (std, по умолчанию) / «монеты» (coins) / «полоска HP» (hp). RUBG всегда hp
