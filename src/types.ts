...
export interface GameMap {
  ...
  zoneSec?: number; // RUBG: ПОЛНОЕ время безопасной зоны (все ожидания + сжатия) в СЕКУНДАХ; нет — 600 (10 мин). Настраивается в редакторе карт: минуты и секунды. Работает ТОЛЬКО пока фазы не заданы вручную (zonePhases)
  zonePhases?: { wait: number; shrink: number; dist: number }[]; // RUBG: АВТОРСКИЕ ФАЗЫ ЗОНЫ (подробная настройка): wait — пауза ДО сжатия (сек), shrink — длительность сжатия (сек), dist — на сколько КЛЕТОК сузится радиус (0 = фаза без сужения). Задана — zoneSec игнорируется (полное время = сумма фаз)
