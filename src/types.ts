...
export const MODE_PRESETS: Record<MapMode, ModePreset> = {
  ...
  quest:     { startMin: 60, startTries: 60, resMode: 'std', smoothMove: true },
  quest1p:   { startMin: 60, startTries: 60, resMode: 'std', smoothMove: true },
...
export type QuestGoalKind = 'boss' | 'tasks' | 'coins' | 'hp' | 'time' | 'tries' | 'none';
export interface QuestGoal {
  kind: QuestGoalKind;      // что должно быть выполнено
  bossId?: string;          // kind='boss': PlacedBoss.id — какой босс должен пасть
  count?: number;           // 'tasks' — сколько заданий; 'coins' — бронзы; 'hp' — % HP; 'time' — секунд; 'tries' — попыток
}
...
export const questGoalText = (g: QuestGoal | undefined, map: Pick<GameMap, 'bossLib' | 'bosses'>): string => {
  ...
  if (g.kind === 'time') return `запас времени ≥ ${Math.round(n / 60)} мин`;
  return `запас попыток ≥ ${n}`;
};
