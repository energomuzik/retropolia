...
export const MAP_MODES: { id: MapMode; name: string; hint: string }[] = [
  ...
  { id: 'journey', name: 'JOURNEY', hint: 'Приключение: все фишки стартуют одновременно и ходят напрямую в любые стороны, кубиков и передачи хода нет. Кто ПЕРВЫМ пересечёт ячейку задания — у того оно открывается, остальные фишки замирают и смотрят трансляцию. Побеждает последний с ресурсами.' },
];
...
export interface GameSession {
  ...
  journeyPos?: Record<string, { x: number; y: number; dir?: 'up' | 'down' | 'left' | 'right'; ts?: number; mv?: boolean }>; // JOURNEY: авторитетные позиции ВСЕХ фишек в px поля (пишет хост); mv — «идёт сейчас» (false = стоит)
...
export const APP_VERSION = 25; // 25: JOURNEY переписан — ВСЕ фишки стартуют и ходят ОДНОВРЕМЕННО, без жеребьёвки кубиков и передачи хода; кто ПЕРВЫМ пересёк ячейку задания — у того открывается задание (ход = игрок задания), остальные замирают и смотрят, после задания все снова ходят; НЕВИДИМЫЕ СТЕНЫ — прямоугольные зоны «куда нельзя» (редактор рисует протягиванием, работают только в JOURNEY, проверка при создании карты); SKILL CHALLENGE — в редакторе запрещены ячейки квизов/бонусов/штрафов/отдыха, а встав на уже пройденную/скипнутую ячейку фишка сама едет к следующей неигранной; плавание фишки на месте убрано
