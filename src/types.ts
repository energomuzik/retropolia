...
export const APP_VERSION = 22; // 22: крестики удаления снова видны (фикс позиционирования), режим удаления действует на ВСЕ удаления (карты, ромы, сохранения, карточки, партии), Ctrl+Z честно возвращает последнее удаление (ластик и Delete-клавиша тоже запоминают), папки ромов в запуске эмулятора; старые партии не продолжаются
