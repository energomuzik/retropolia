...
function buildHtml(core: string, volume: number, base: string, bootStateB64: string | null, nonce: string, remapJson: string): string {
  ...
  return [
    ...
    '    }',
    '  }catch(err3){}',
    // Глушим РОДНЫЕ геймпад-привязки ядра для наших индексов (value2="" = отключено в EmulatorJS):
    // иначе ядро параллельно читает пад по своим дефолтам и дублирует наш «насос».
    '  try{',
    '    var e2=window.EJS_emulator;',
    '    if(e2){',
    '      var objs=[e2.controls,e2.settings&&e2.settings.controls,e2.gameManager&&e2.gameManager.controls];',
    '      for(var oi=0;oi<objs.length;oi++){',
    '        var oc=objs[oi];',
    '        if(!oc)continue;',
    '        var op=oc[0]||oc["0"];',
    '        if(!op)continue;',
    '        for(var oj=0;oj<remapSpec.length;oj++){',
    '          var ori=remapSpec[oj].idx;',
    '          var oe=op[ori]||op[String(ori)];',
    '          if(oe){oe.value2="";}',
    '        }',
    '      }',
    '    }',
    ...
    // Крестовина дополнительно дублируется ЛЕВЫМ СТИКОМ: многие падки (и Xbox-крестовина
    // в части браузеров) отдают направления осями, а не кнопками 12..15.
    // Родные геймпад-привязки ядра для наших действий глушатся в buildKeyMap (value2="").
    '(function(){',
    'var prevPressed={};',
    ...
    '    if(prevPressed[act]&&PAD_ACTION_TO_RETRO[act]!==undefined){try{g.simulateInput(0,PAD_ACTION_TO_RETRO[act],0);}catch(e){}}',
    '    prevPressed[act]=false;',
    '  }',
    '}',
    'function pumpGamepad(){',
    '  var g=window.EJS_emulator&&window.EJS_emulator.gameManager;',
    '  if(!g||typeof g.simulateInput!=="function"){requestAnimationFrame(pumpGamepad);return;}',
    '  var pads;',
    ...
    '  var stick={UP:sy<-0.5,DOWN:sy>0.5,LEFT:sx<-0.5,RIGHT:sx>0.5};',
    '  var pressedNow={};',
    '  for(var act in PAD_ACTION_TO_RETRO){',
    '    var retroIdx=PAD_ACTION_TO_RETRO[act];',
    '    if(retroIdx===undefined||retroIdx===null)continue;',
    '    var pressed=false;',
    '    var phys=prefs?prefs[act]:null;',
    '    if(phys!==undefined&&phys!==null&&phys>=0&&phys<buttons.length){',
    '      var b=buttons[phys];',
    '      var val=(typeof b==="object")?b.value:b;',
    '      pressed=(val!==undefined&&val!==null&&val>0.5)||(val===true);',
    '    }',
    '    if(!pressed&&DIRS[act]&&stick[act])pressed=true;',
    '    pressedNow[act]=pressed;',
    '    var was=prevPressed[act];',
