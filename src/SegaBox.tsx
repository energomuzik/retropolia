...
function buildHtml(core: string, volume: number, base: string, bootStateB64: string | null, nonce: string, remapJson: string): string {
  ...
  return [
    ...
    'function applyRemap(){',
    '  remapByKey={};remapByCode={};var n=0;',
    '  for(var i=0;i<remapSpec.length;i++){',
    '    var it=remapSpec[i];',
    '    var from=String(it.from||"").toLowerCase();',
    '    var to=String(it.to||"").toLowerCase();',
    '    if(!from||!to||from===to){continue;}',
    '    remapByKey[from]=to;n++;',
    '    var fc=codeOf(from);',
    '    if(fc){remapByCode[fc.toLowerCase()]=to;}',
    '  }',
