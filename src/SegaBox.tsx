...
function buildHtml(core: string, volume: number, base: string, bootStateB64: string | null, nonce: string, remapJson: string): string {
  ...
  return [
    ...
    // опрос геймпада: читаем кнопки и передаём нажатия в эмулятор через simulateInput
    'var gpadSpec=[];var lastGpadState={};',
    'function pollGamepad(){',
    '  try{',
    '    var gps=navigator.getGamepads();if(!gps||!gps[0]){return;}',
    '    var gp=gps[0];if(!gp){return;}',
    '    for(var i=0;i<gpadSpec.length;i++){',
    '      var it=gpadSpec[i];var btnIdx=it.btn;if(btnIdx===undefined||btnIdx===null)continue;',
    '      var b=gp.buttons[btnIdx];if(!b)continue;',
    '      var pressed=b.pressed||((typeof b==="number")?b>0.5:false);',
    '      var prev=!!lastGpadState[btnIdx];',
    '      if(pressed!==prev){',
    '        lastGpadState[btnIdx]=pressed;',
    '        simBtn(it.idx,pressed);',
    '      }',
    '    }',
    '  }catch(e){}',
    '}',
    ...
    '  var d=e.data||{};',
    '  if(d.type==="set-remap-spec"){remapSpec=d.spec||[];gpadSpec=[];for(var i=0;i<remapSpec.length;i++){var it=remapSpec[i];if(it.btn!==undefined&&it.btn!==null){gpadSpec.push({idx:it.idx,btn:it.btn});}}remapTries=0;remapPump();return;}',
