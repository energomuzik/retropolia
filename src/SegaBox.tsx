...
function buildHtml(core: string, volume: number, base: string, bootStateB64: string | null, nonce: string, remapJson: string, chaosJson: string): string {
  ...
  return [
    ...
    '  return out;',
    '}',
    // оверлей помех/VHS: рисуется поверх кадра отдельным канвасом (~15 к/с)
    'var fxTimer=0;',
    'function drawFxOverlay(){',
    '  try{',
    '    var ov=document.getElementById("chaos-fx");if(!ov){return;}',
    '    var host=document.getElementById("game");var cv=host&&host.querySelector("canvas");',
    '    var w=ov.clientWidth||320,h=ov.clientHeight||240;',
    '    if(ov.width!==w||ov.height!==h){ov.width=w;ov.height=h;}',
    '    var x2=ov.getContext("2d");if(!x2){return;}',
    '    var fx=chaosFx();',
    '    x2.clearRect(0,0,w,h);',
    '    if(fx.noise){',
    '      var img=x2.createImageData(w,h);var dd=img.data;',
    '      for(var i2=0;i2<dd.length;i2+=4){var vv=(Math.random()*255)|0;dd[i2]=vv;dd[i2+1]=vv;dd[i2+2]=vv;dd[i2+3]=Math.random()<0.5?24:66;}',
    '      x2.putImageData(img,0,0);',
    // разрыв строки: случайная полоса кадра сдвигается вбок (как дрожащая антенна)
    '      if(cv&&Math.random()<0.55){var sh=h*0.05+Math.random()*h*0.1;var sy=Math.random()*Math.max(1,h-sh);var dx=(Math.random()*2-1)*w*0.07;x2.drawImage(cv,0,sy*cv.height/h,cv.width,sh*cv.height/h,dx,sy,w,sh);}',
    '    }',
    '    if(fx.vhs){',
    // дрожание строк — белёсые горизонтальные полоски
    '      var nn=2+((Math.random()*3)|0);',
    '      for(var j=0;j<nn;j++){x2.fillStyle="rgba(255,255,255,0.10)";x2.fillRect(0,Math.random()*h,w,1+Math.random()*2);}',
    // движущаяся полоса трекинга (светлая + тёмная кромка) со сдвигом строки
    '      var tp=((Date.now()/2600)%1)*h;var th=Math.max(6,h*0.10);',
    '      if(cv){x2.globalAlpha=0.5;x2.drawImage(cv,0,tp*cv.height/h,cv.width,th*cv.height/h,(Math.random()*2-1)*w*0.025,tp,w,th);x2.globalAlpha=1;}',
    '      x2.fillStyle="rgba(255,255,255,0.15)";x2.fillRect(0,tp,w,th*0.5);',
    '      x2.fillStyle="rgba(0,0,0,0.28)";x2.fillRect(0,tp+th*0.5,w,th*0.5);',
    // виньетка по краям — «затёртая плёнка»
    '      var gr=x2.createRadialGradient(w/2,h/2,h*0.35,w/2,h/2,h*0.85);',
    '      gr.addColorStop(0,"rgba(0,0,0,0)");gr.addColorStop(1,"rgba(0,0,0,0.5)");',
    '      x2.fillStyle=gr;x2.fillRect(0,0,w,h);',
    '    }',
    '  }catch(e){}',
    '}',
    'function applyChaos(){',
    ...
    '      }',
    '    }',
    // оверлей помех/VHS — создаётся/убирается по списку пакостей
    '    var needFx=fx.noise||fx.vhs;',
    '    var ov=document.getElementById("chaos-fx");',
    '    if(needFx&&!ov&&host){ov=document.createElement("canvas");ov.id="chaos-fx";ov.style.cssText="position:absolute;inset:0;width:100%;height:100%;z-index:38;pointer-events:none";host.appendChild(ov);}',
    '    if(ov){ov.style.display=needFx?"block":"none";}',
    '    if(needFx&&!fxTimer){fxTimer=setInterval(drawFxOverlay,66);drawFxOverlay();}',
    '    if(!needFx&&fxTimer){clearInterval(fxTimer);fxTimer=0;}',
    ...
    // помехи на кадре трансляции: белые точки + разрыв строки
    '      if(fx2.noise){',
    '        for(var i3=0;i3<340;i3++){var v3=(180+Math.random()*75)|0;x2.fillStyle="rgba("+v3+","+v3+","+v3+","+(Math.random()*0.5).toFixed(2)+")";x2.fillRect(Math.random()*c2.width,Math.random()*c2.height,1+Math.random()*2,1+Math.random()*2);}',
    '        var slh=c2.height*0.05+Math.random()*c2.height*0.08;var sly=Math.random()*Math.max(1,c2.height-slh);var sldx=(Math.random()*2-1)*c2.width*0.06;',
    '        x2.drawImage(c2,0,sly,c2.width,slh,sldx,sly,c2.width,slh);',
    '      }',
    // VHS на кадре: полоса трекинга + дрожание строк + виньетка
    '      if(fx2.vhs){',
    '        var tp2=((Date.now()/2600)%1)*c2.height;var th2=Math.max(4,c2.height*0.1);',
    '        x2.globalAlpha=0.5;x2.drawImage(c2,0,tp2,c2.width,th2,(Math.random()*2-1)*c2.width*0.025,tp2,c2.width,th2);x2.globalAlpha=1;',
    '        x2.fillStyle="rgba(255,255,255,0.14)";x2.fillRect(0,tp2,c2.width,th2*0.5);',
    '        x2.fillStyle="rgba(0,0,0,0.26)";x2.fillRect(0,tp2+th2*0.5,c2.width,th2*0.5);',
    '        for(var j2=0;j2<3;j2++){x2.fillStyle="rgba(255,255,255,0.10)";x2.fillRect(0,Math.random()*c2.height,c2.width,1);}',
    '        var gr2=x2.createRadialGradient(c2.width/2,c2.height/2,c2.height*0.35,c2.width/2,c2.height/2,c2.height*0.85);',
    '        gr2.addColorStop(0,"rgba(0,0,0,0)");gr2.addColorStop(1,"rgba(0,0,0,0.5)");',
    '        x2.fillStyle=gr2;x2.fillRect(0,0,c2.width,c2.height);',
    '      }',
    '      x2.filter="none";',
