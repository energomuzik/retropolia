...
function buildHtml(core: string, volume: number, base: string, bootStateB64: string | null, nonce: string, remapJson: string): string {
  ...
  return [
    ...
    // отправляем нажатия через gameManager.simulateInput по привязкам из DEFAULT_GPAD (input.ts)
    '(function(){',
    'var lastButtons=null;',
    'var prevPressed={};',
    'function getGpadPrefs(){',
    '  try{',
    '    var raw=localStorage.getItem("retropolia-emu-prefs");',
    '    if(raw){var p=JSON.parse(raw);if(p&&p.gpad)return p.gpad;}',
    '  }catch(e){}',
    '  return {UP:12,DOWN:13,LEFT:14,RIGHT:15,A:0,B:1,SELECT:8,START:9};',
    '}',
    ...
    '  if(!gp){lastButtons=null;requestAnimationFrame(pumpGamepad);return;}',
    '  var prefs=getGpadPrefs();',
    '  var actionToIdx={};',
    '  for(var k in prefs){actionToIdx[k]=prefs[k];}',
    '  var buttons=gp.buttons;',
    '  if(!buttons){requestAnimationFrame(pumpGamepad);return;}',
    '  var pressedNow={};',
    '  for(var act in actionToIdx){',
    '    var btnIdx=actionToIdx[act];',
    '    if(btnIdx===undefined||btnIdx===null||btnIdx>=buttons.length)continue;',
    '    var b=buttons[btnIdx];',
    '    var val=(typeof b==="object")?b.value:b;',
    '    var isPressed=(val!==undefined&&val!==null&&val>0.5)||(val===true);',
    '    pressedNow[act]=isPressed;',
    '    var was=prevPressed[act];',
    '    if(isPressed&&!was){g.simulateInput(0,btnIdx,1);}',
    '    else if(!isPressed&&was){g.simulateInput(0,btnIdx,0);}',
    '  }',
